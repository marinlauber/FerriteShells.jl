using FerriteShells,LinearAlgebra,WriteVTK,TimerOutputs
using Printf
...
function get_pillow_grid()
    # load the deformed geometry
    # fname = "/home/marin/Workspace/HHH/code/square_pillow/square_pillow_N8_S8/geom.inp"
    fname = "/home/marin/Workspace/HHH/code/square_pillow/pillow_Q9.inp"
    grid = get_ferrite_grid(fname)
    addfacetset!(grid, "edge",  x -> isapprox(x[1], 0.5, atol=1e-6) || isapprox(x[2], 0.5, atol=1e-6))
    addfacetset!(grid, "sym_x", x -> isapprox(x[1], 0.0, atol=1e-10))
    addfacetset!(grid, "sym_y", x -> isapprox(x[2], 0.0, atol=1e-10))
    addnodeset!(grid, "center", x -> norm(x) < 1e-10)
    return grid
...
# function ferritegmsh_grid()
#     fname = "/home/marin/Workspace/HHH/code/square_pillow/geom.geo"
#     grid = togrid(fname)
#     addfacetset!(grid, "edge",  x -> isapprox(x[1], 0.5, atol=1e-6) || isapprox(x[2], 0.5, atol=1e-6))
#     addfacetset!(grid, "sym_x", x -> isapprox(x[1], 0.0, atol=1e-10))
#     addfacetset!(grid, "sym_y", x -> isapprox(x[2], 0.0, atol=1e-10))
