...
function max_principal_stress_qp(dh, scv, u, mat)
    ...
    for cell in CellIterator(dh)
        ...
        @inbounds for qp in 1:n_qp
            a₁, a₂ = FerriteShells.covariant_basis(scv, qp, u_e, n_nodes)
...
# material
mat = LinearElastic(1.0e6, 0.3, 0.002)
ρ   = 1200.0       # density [kg/m³]

# grid and interpolation space with shear treatment of the shell
...
# nodal projection of the integration point solution
...
for step in 1:n_steps
    ...
    for iter in 1:max_iter
        ...
        rhs .= -(M * a_new .+ (1 - α_hht) .* (α_damp .* (M * v_new) .+ r_int .- p_new .* F_plv) .+ α_hht .* g_old)
        apply_zero!(rhs, ch)
