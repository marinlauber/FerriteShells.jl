...
# {SRF_1..8} and {SRF_8..16} together form the closed device; SRF_1/SRF_8 are the
...
meshfile = "/home/marin/Workspace/HHH/code/miniLIMO/p6/geom.inp"
...
ρ   = 1200.0
mat = LinearElastic(20e6, 0.3, 0.001)
...
# Morph BC edge: the y=0 base rim of the endocardium patches SRF_1/SRF_8 — the
...
base8 = base_nodes(8)                    # SRF_8 base rim (folds −z)
addnodeset!(grid, "base1", base1)
addnodeset!(grid, "base8", base8)
addnodeset!(grid, "base8_only", setdiff(base8, base1))   # drop the 2 shared corner ids
@printf("  morph edges: SRF_1 base %d, SRF_8 base %d (shared %d)\n",
        length(base1), length(base8), length(intersect(base1, base8)))
...
# SRF_8 down (zsign=−1).  Δx → u_x (same for both, rims stay coincident in-plane),
# ellipse height → ±u_z; u_y = 0; θ = 0.  The shared corner ids (u_z=0 there) are
# constrained once via base1; base8_only excludes them to avoid a duplicate DOF.
...
morph_down = generate_boundary_function(grid, "base8"; ramp=ramp, zsign=-1)

ch = ConstraintHandler(dh)
add!(ch, Dirichlet(:u, getnodeset(grid, "base1"),      (x, t) -> morph_up(x, t),   [1, 3]))
add!(ch, Dirichlet(:u, getnodeset(grid, "base8_only"), (x, t) -> morph_down(x, t), [1, 3]))
add!(ch, Dirichlet(:u, getnodeset(grid, "base1"),      x -> 0.0, [2]))
add!(ch, Dirichlet(:u, getnodeset(grid, "base8_only"), x -> 0.0, [2]))
add!(ch, Dirichlet(:θ, getnodeset(grid, "base1"),      x -> zeros(2), [1, 2]))
add!(ch, Dirichlet(:θ, getnodeset(grid, "base8_only"), x -> zeros(2), [1, 2]))
close!(ch); Ferrite.update!(ch, 0.0)     # start flat; the morph ramps over Phase 1
edge_nodes = union(base1, base8)         # for the VTK edge marker
...
# lower half {SRF_8..16}) flips the mirror half (as in limo_dynamic_full.jl):
...
endo_srf = srfset((1, 8))
ext_srf  = srfset((2, 3, 4, 9, 10, 11))
foot_srf = srfset((5, 6, 7, 12, 13, 14))
...
for k in 8:14, cid in getcellset(grid, "SRF_$k"); psign[cid] = -1.0; end
