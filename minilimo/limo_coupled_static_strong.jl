...
# Strongly (monolithically) coupled STATIC miniLIMO: quasi-static structure + 0D Windkessel
# solved together in ONE Newton system per time step.  Static counterpart of
# `limo_coupled_dynamic_strong.jl` (drop the M·ä inertia and Rayleigh damping; keep the same
# implicit-Euler 0D block).  Complements the static weakly-coupled `limo_coupled_static_weak.jl`
# (Lie–Trotter split with a black-box ODE integrator), filling the static×strong matrix cell.
#
#   PHASE 1 — dynamic morph  (t ∈ [0, T_sim], HHT-α)   [identical to the other coupled files]
#     Morph the edge onto the elliptic arc + fill Plv → morphed state u = un, Plv=p_max.
#     (Only the morphed displacement `un` is carried forward; velocity/acceleration are dropped.)
#
#   PHASE 2 — monolithic strong coupling  (quasi-static structure, implicit-Euler 0D, Δt = dt_cpl)
#     The Windkessel is discretized by implicit Euler and embedded in the structural Newton; the
#     structure carries NO inertia (equilibrium each step).  The LV chamber volume is NOT an
#     independent 0D state — it is V_LV(u) = 2·V₃D(u); Plv is the pressure that closes the flow
#     balance.  Unknowns per step: (u ∈ Rᴺ, Plv, Pa, Pv).  Residuals:
#       R_s(u,Plv) = r_int(u) − F_ext(u,Plv,Pact)                                    [N]
...
#       K_eff = K_int − Plv·K_plv − Pact·K_pact + Pact·K_plvpact  (no mass/damping),
#       v1 = K_eff⁻¹(−R_s),  v2 = K_eff⁻¹F_plv  (∂R_s/∂Plv = −F_plv),  δu = v1 + δPlv·v2,
...
pvd = paraview_collection("minilimo-coupled-static-strong")
vtk_step = Ref(0)
resu = zeros(3, getnnodes(dh.grid))
resθ = zeros(2, getnnodes(dh.grid))
d, G3 = director_field(dh, scv, u)
VTKGridFile("minilimo-coupled-static-strong-0", dh) do vtk
...
#             end
#             d, G3 = director_field(dh, scv, u)
#             VTKGridFile("minilimo-coupled-static-strong-$(vtk_step[])", dh) do vtk
#                 write_solution(vtk, dh, u)
#                 Ferrite.write_node_data(vtk, resu, "ru")
#                 Ferrite.write_node_data(vtk, resθ, "rθ")
#                 Ferrite.write_node_data(vtk, d,  "director")
...
bufs_cpl = (; K_int, r_int, K_plv, F_plv, K_pact, F_pact, K_plvpact, F_plvpact, K_eff,
              res, rhs, v1, v2, dVdu, F_lu, free, sdofs, ke, re, u_e)

# Monolithic strong-coupling step (STATIC structure): solve the quasi-static equilibrium AND
# the implicit-Euler 0D Windkessel simultaneously for (u, Plv, Pa, Pv).  `u` updated in place;
# the LV chamber volume is V_LV(u) = 2·V₃D(u), Plv is the coupling multiplier.  V_LVₙ/Paₙ/Pvₙ
# are the previous converged 0D state.  Returns (Plv, Pa, Pv, iters, converged, V₃D).
function solve_coupled_static_strong_step!(u, Plv, Pa, Pv, V_LVₙ, Paₙ, Pvₙ,
                                    Pact, Δt, dh, scv, mat, ch,
                                    Plv_srf, Pact_srf, PlvPact_srf, wk, bufs;
                                    max_iter=20, tol=1e-4, verbose=false)
    (; Ra, Rp, Rv, Ca, Cv, Pscale) = wk
    (; K_int, r_int, K_plv, F_plv, K_pact, F_pact, K_plvpact, F_plvpact, K_eff,
       res, rhs, v1, v2, dVdu, F_lu, free, sdofs, ke, re, u_e) = bufs
    ...
    for iter in 1:max_iter
        ...
        assemble_pressure_region!(K_pact,    F_pact,    dh, scv, u, Pact_srf,    sdofs, ke, re, u_e)
        assemble_pressure_region!(K_plvpact, F_plvpact, dh, scv, u, PlvPact_srf, sdofs, ke, re, u_e)
        # static structural residual R_s = r_int − F_ext (no inertia, no damping)
        @. res = r_int - (Plv * F_plv + Pact * F_pact - Pact * F_plvpact)
        apply_zero!(res, ch)
        ...
        # static structural tangent + two back-substitutions (shared with the volume row)
        K_eff.nzval .= K_int.nzval .- Plv .* K_plv.nzval .-
                       Pact .* K_pact.nzval .+ Pact .* K_plvpact.nzval
        @. rhs = -res
        apply_zero!(K_eff, rhs, ch)
        lu!(F_lu, K_eff)
        ldiv!(v1, F_lu, rhs)
        ldiv!(v2, F_lu, F_plv)   # ∂R_s/∂Plv = −F_plv → δu = v1 + δPlv·v2
...
println("\nPHASE 2 — static monolithic strong 3D-0D coupling (dt_cpl=$(dt_cpl) s)")
...
@time let V_LVₙ = V_LV0, Paₙ = Pa0, Pvₙ = Pv0, t_cpl = 0.0, Plv = p_max, Pa = Pa0, Pv = Pv0
    ...
    while t_cpl < T_beat - dt_cpl / 2
        ...
        Pact_mmHg = 600 * ϕᵢ(t_cpl; tC=0.1, tR=0.4, TC=0.3, TR=0.3)
        Pact = Pact_mmHg / Pa2mmHg

        # quasi-static: warm-start from the previous equilibrium, morph BC frozen.
        apply!(u, ch)

        Plv, Pa, Pv, n_iter, converged, V₃D = solve_coupled_static_strong_step!(
            u, Plv, Pa, Pv, V_LVₙ, Paₙ, Pvₙ, Pact, dt_cpl, dh, scv, mat, ch,
            Plv_srf, Pact_srf, PlvPact_srf, wk, bufs_cpl; max_iter=max_iter, tol=tol_cpl, verbose=false)

        # advance 0D history (no structural inertia to carry)
...
plot(p1, p2)
# savefig("minilimo-coupled-static-strong-N$Np.png")
