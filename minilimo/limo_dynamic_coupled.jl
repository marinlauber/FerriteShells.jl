...
# Dynamic (HHT-α) morphing warm-up followed by a 3D-0D Windkessel-coupled beat, on the
# rectangular multi-surface mesh built by `make_minilimo_grid` (same geometry as
# `limo_inflation.jl` / `limo_dynamic.jl`).  Two phases:
#
#   PHASE 1 — dynamic morph  (t ∈ [0, T_sim], HHT-α)
#     Edge nodes are driven from the flat reference onto the target elliptic arc while a
#     follower Plv (same ramp) fills the endocardium (SRF_1 ∪ SRF_2).  Mass-proportional
#     Rayleigh damping C = α_damp·M drives the transient to the morphed, Plv-filled state.
#     End state: u = un, Plv = p_max.
#
#   PHASE 2 — 3D-0D coupling  (quasi-static, Lie–Trotter split against a 0D Windkessel)
#     Starting from `un`, the 0D Windkessel ODE is advanced by dt_cpl with Plv held; its
#     LV volume gives a target V that the 3D shell must enclose.  A volume-controlled
#     Schur-complement (bordered-Newton) solve finds (u, Plv) so that R_int(u) = F_ext and
#     V₃D(u) = V_target, treating Plv as the multiplier; the converged Plv is fed back into
#     the ODE.  Three follower surfaces:
...
#
# HHT-α (Phase 1) with damping and follower pressure:
#   g(u,v,p) = C·v + r_int(u) − p·F_p(u)
#   R = M·ä_{n+1} + (1−α)·g_{n+1} + α·g_old = 0
#   γ = ½ − α,  β = (1−α)²/4
#   K_eff = M·[1/(βΔt²) + (1−α)·α_damp·γ/(βΔt)] + (1−α)·(K_int − p·K_plv)
...
# Follower-pressure load vector + tangent restricted to `cellset` (unit magnitude Pᵢ).
...
u_trial   = zeros(N_dof)
rhs       = zeros(N_dof)
rhs1      = zeros(N_dof)   # coupled-phase Newton RHS
...
bufs_cpl = (; K_int, r_int, K_plv, F_plv, K_pact, F_pact, K_plvpact, F_plvpact,
              K_eff, rhs1, v1, v2, dVdu, F_lu, sdofs, ke, re, u_e)

# Quasi-static volume-controlled Schur-complement (bordered-Newton) solve for one
# coupling step: find (u, p=Plv) with R_int(u) = F_ext(u) and V₃D(u) = V_target,
...
@time let u = copy(un), p = p_max
    ...
    while integrator.t < tspan[2] - dt_cpl / 2
        ...
        for cell in CellIterator(dh)
            sd = shelldofs(cell)
            for (I, nid) in enumerate(cell.nodes)
                resu[:, nid] .= rhs1[sd[5I-4:5I-2]]
                resθ[:, nid] .= rhs1[sd[5I-1:5I  ]]
            end
        end
        d, G3 = director_field(dh, scv, u)
        VTKGridFile("minilimo-dynamic-coupled-$(vtk_step[])", dh) do vtk
