...
addcellset!(grid, "Pact", x -> (abs(x[1]) < 0.035 && 0.004 < x[2] < 0.09) )
...
pvd = paraview_collection("minilimo-2")
vtk_step = Ref(0)
...
vol = -2compute_volume(dh, scv, un) * m3_to_ml
...
@time let u = copy(un), p = p_max, k₀ = length(pvd.timeSteps)
    ...
    while integrator.t < tspan[2] - dt_cpl / 2
        ...
        if mod(step, 5) == 0
            VTKGridFile("minilimo-2-$(vtk_step[])", dh) do vtk
                vtk_step[] += 1
...
p2=plot(vols, pres, label=:none, xlim=(250,350),ylims=(0, 100),
